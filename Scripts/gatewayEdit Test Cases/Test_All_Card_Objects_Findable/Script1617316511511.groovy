...
folders = ['Blue_Banners','Card_tN','Card_Scripture','Card_tA','Card_tQ','Card_tW_Article','Card_tW_List']
...
//return false
	...
	for (object in list) {
		...
		myObject = folder + '/' + object
		
		if (!object.contains('Parmed')) {
			println('testing ' + myObject)
			
			if (object.startsWith('button') || object.startsWith('comboBoxArrow') || object.startsWith('menu')) {
				println('clicking ' + myObject)
				WebUI.click(findTestObject(myObject))
				WebUI.delay(1)
				if (object.startsWith('menu')) {
					WebUI.click(findTestObject('Card_Settings/button_Settings_Close'))
					WebUI.delay(1)
				}
			} else if (object.startsWith('input') || object.startsWith('text')) {
				text = WebUI.getText(findTestObject(myObject))
				println(myObject + ' text is [' + text + ']')
			}
			if(myObject == 'Card_tN/button_tN_Next' && 1 == 2) {
				return false
			}
		}
	}
