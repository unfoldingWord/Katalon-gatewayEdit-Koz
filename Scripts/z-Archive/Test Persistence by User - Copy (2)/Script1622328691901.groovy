		...
		while (loop <= loopMax) {
			...
			cards.each { id, font ->
				...
				if (font == 'Markdown') {
					...
					if (user == 0) {
						if (myOrg == 0) {
							markdowns00 = markdowns
						} else {
							markdowns01 = markdowns
						}
					} else {
						if (myOrg == 0) {
							markdowns10 = markdowns
						} else {
							markdowns11 = markdowns
						}
					}
				...
				if (user == 0) {
					if (myOrg == 0) {
						fontSizes00 = fontSizes
					} else {
						fontSizes01 = fontSizes
					}
				} else {
					if (myOrg == 0) {
						fontSizes10 = fontSizes
					} else {
						fontSizes11 = fontSizes
					}
				}
				...
				if (id == 'tn') {
					...
					if (user == 0) {
						if (myOrg == 0) {
							columnsMap00 = columnsMap
						} else {
							columnsMap01 = columnsMap
						}
					} else {
						if (myOrg == 0) {
							columnsMap10 = columnsMap
						} else {
							columnsMap11 = columnsMap
						}
					}
...
// Now test for persistence after changing users
users.each { user ->
...
def testByUserOrg(user, org) {
	if (user == 0 && org == 0) {
		markdowns = markdowns00
		fontSizes = fontSizes00
		columnsMap = columnsMap00
	} else if (user == 0 && org == 1) {
		markdowns = markdowns01
		fontSizes = fontSizes01
		columnsMap = columnsMap01
	} else if (user == 1 && org == 0) {
		markdowns = markdowns10
		fontSizes = fontSizes10
		columnsMap = columnsMap10
	} else if (user == 1 && org == 1) {
		markdowns = markdowns11
		fontSizes = fontSizes11
		columnsMap = columnsMap11
	}
	
	
	WebUI.callTestCase(findTestCase('Components/LogIn'), [('user') : users[user], ('organization'): orgs[org]])
